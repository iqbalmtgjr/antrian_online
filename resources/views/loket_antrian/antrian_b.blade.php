    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="page-title-box">
                <h4 class="page-title">Loket Antrian B</h4>
                <form action="">
                    ...
                    <div id="result">
                        <table id="" class="table">
                            <div class="row">
                                ...
                                <a href="{{ url('/antrian/cetak_b') }}" class="btn btn-primary btn-md ml-3 mb-3 ">
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel">Antrian Loket B</h5>
                                ...
                                <div class="modal-body">
                                    <form action="{{ url('/antrian/lanjut/b') }}" method="post">
                                        ...
                                        <center>
                                            @if ($data->count() >= 9)
                                                <h2>
                                                    B{{ App\Models\Antrian::where('id_pelayanan', 2)->get()->last()->no_antrian + 1 }}
                                            ...
                                            @else
                                                <h2>
                                                    @if ($data->count() == 0)
                                                        @if ($data->count() == 0 &&
        App\Models\Laporan::where('id_pelayanan', 2)->where('tgl_antrian', $tgl)->get()->count() == 0)
                                                            B0{{ $data->count() + 1 }}
                                                        @else
                                                            @if (App\Models\Laporan::where('id_pelayanan', 2)->where('tgl_antrian', $tgl)->get()->count() >= 9)
                                                                B{{ App\Models\Laporan::where('id_pelayanan', 2)->where('tgl_antrian', $tgl)->get()->last()->no_antrian + 1 }}
                                                            @else
                                                                B0{{ App\Models\Laporan::where('id_pelayanan', 2)->where('tgl_antrian', $tgl)->get()->last()->no_antrian + 1 }}
                                                            @endif
                                                        @endif
                                                    @elseif (App\Models\Antrian::where('id_pelayanan', 2)->count() < 1)
                                                        B0{{ $data->count() + 1 }} @else @if (App\Models\Antrian::where('id_pelayanan', 2)->get()->last()->no_antrian >= 9)
                                                            B{{ App\Models\Antrian::where('id_pelayanan', 2)->get()->last()->no_antrian + 1 }}
                                                        @else
                                                            B0{{ App\Models\Antrian::where('id_pelayanan', 2)->get()->last()->no_antrian + 1 }}
                                                    @endif
                                            @endif
                                            </h2>
                                            {{-- @endif --}}
